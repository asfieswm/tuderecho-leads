...
def infer_step_type(cfg_row) -> str:
    ...
    has_opts = (
        ...
        or bool((cfg_row.get("Siguiente_Si_2") or "").strip())
    )
    has_text = (
    ...
    try:
        ...
        if msg_lower == "menu":
            if (lead.get("Abogado_Asignado_ID") or "").strip() or procesar_status == "DONE":
                ...
                out = get_text(cfg.get("CLIENTE_MENU", {})) or (
                    f"Hola {nombre} 👋\n"
                    "1️⃣ Próximas fechas\n2️⃣ Resumen\n3️⃣ Contactar a mi abogada"
                )
                out = out.replace("{Nombre}", nombre or "")
                log(ws_logs, lead_id, "CLIENTE_MENU", msg_in_raw, out, telefono=from_phone)
                return twiml(out)
        ...
        # ==========================================================
        # INTERCEPCIÓN: ESPERANDO_LLAMADA_OPCION
        # ==========================================================
        ...
        if estatus == "AI_EMPATIA":
            if msg_opt in ("1", "2"):
                ...
                update_row_cells(ws_leads, lead_row, {
                    "Tipo_Caso":            msg_opt,
                    "ESTATUS":              nxt_emp,
                    "Paso_Anterior":        "AI_EMPATIA",
                    "Ultima_Actualizacion": now_iso(),
                }, hmap=h)
                out = get_text(cfg.get(nxt_emp, {})) or "Para comenzar, ¿podrías indicarme tu nombre, por favor?"
                out = out.replace("{Nombre}", nombre or "")
        ...
        if estatus == "INICIO" and msg_opt not in ("1", "2"):
            ...
            out = out.replace("{Nombre}", nombre or "")
            log(ws_logs, lead_id, "INICIO", msg_in_raw, out, telefono=from_phone)
            return twiml(out)
        ...
        if t == "OPCIONES":
            ...
            if nxt == "EN_PROCESO":
                ...
                if q is not None:
                    try:
                        ...
                        app.logger.info(
                            f"[RQ] Encolado process_lead lead_id={lead_id} "
                            f"job_id={job.get_id()} cola={REDIS_QUEUE_NAME}"
                        )
                    except Exception as e:
                        app.logger.exception(f"[RQ] No se pudo encolar process_lead: {e}")
                        update_row_cells(ws_leads, lead_row, {
                            "Procesar_AI_Status": "ERROR_ENQUEUE",
                            "Ultimo_Error":       f"ENQUEUE_ERROR: {type(e).__name__}: {e}"
                        }, hmap=h)
                        log(ws_logs, lead_id, "EN_PROCESO", msg_in_raw, out,
                            telefono=from_phone, err=f"enqueue_fail: {type(e).__name__}: {e}")
                else:
                    update_row_cells(ws_leads, lead_row, {
                ...
                out = out.replace("{Nombre}", nombre or "")
                log(ws_logs, lead_id, "EN_PROCESO", msg_in_raw, out, telefono=from_phone)
                return twiml(out)
            ...
            log(ws_logs, lead_id, nxt, msg_in_raw, out, telefono=from_phone)
            return twiml(out)
        ...
        if t == "TEXTO":
            ...
            if not ok:
                log(ws_logs, lead_id, estatus, msg_in_raw, msg_err,
                    telefono=from_phone, err="invalid_text")
                return twiml(msg_err)
            ...
            if nxt == "AI_EMPATIA":
                ...
                if q is not None:
                    try:
                        from worker_jobs import process_caso_libre
                        job_emp = q.enqueue(process_caso_libre, lead_id, job_timeout=90)
                        app.logger.info(
                            f"[RQ] process_caso_libre encolado "
                            f"lead_id={lead_id} job_id={job_emp.get_id()}"
                        )
                    except Exception as e:
                        app.logger.exception(
                            f"[RQ] No se pudo encolar process_caso_libre: {e}"
                        )
                else:
                    app.logger.warning(
...
def api_report():
    ...
    try:
        ...
        try:
            ...
            ini = _date.fromisoformat(ini_str) if ini_str else None
            fin = _date.fromisoformat(fin_str) if fin_str else None
